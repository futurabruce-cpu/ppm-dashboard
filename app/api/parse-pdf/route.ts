  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a data extraction assistant. Extract structured fields from job card PDFs. Return ONLY valid JSON with these exact keys: site_name, site_address, service_date (YYYY-MM-DD format), engineer_name, sheet_type (use "voca" for GOW/Voca Fire jobs, "lfl" for Ladrillos jobs, "other" for anything else). If a field cannot be found, use null.',
        },
        {
          role: 'user',
          content: [
