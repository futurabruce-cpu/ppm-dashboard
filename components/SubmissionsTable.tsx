          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-5 py-3 font-semibold text-gray-600 hidden md:table-cell">Address</th>
                {profile.role !== 'engineer' && (
                  <th className="text-left px-5 py-3 font-semibold text-gray-600 hidden lg:table-cell">Engineer</th>
                )}
                {profile.role === 'superadmin' && (
                  <th className="text-left px-5 py-3 font-semibold text-gray-600 hidden lg:table-cell">Company</th>
                )}
            ...
            <tbody>
              {submissions.map((s, i) => (
                <tr key={s.id} className={`border-b border-gray-50 hover:bg-amber-50/40 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-50/30'}`}>
                  ...
                  <td className="px-5 py-3 text-gray-500 hidden md:table-cell max-w-[200px] truncate">{s.site_address ?? '—'}</td>
                  {profile.role !== 'engineer' && (
                    <td className="px-5 py-3 text-gray-600 hidden lg:table-cell">{s.profiles?.full_name ?? '—'}</td>
                  )}
                  {profile.role === 'superadmin' && (
                    <td className="px-5 py-3 text-gray-600 hidden lg:table-cell">{s.companies?.name ?? '—'}</td>
                  )}
                  ...
                  <td className="px-5 py-3 text-right space-x-2 whitespace-nowrap">
                    <Link
                      ...
                      className="inline-block px-3 py-1 rounded-lg text-xs font-bold text-black transition-colors"
                    ...
                    {s.pdf_url && (
                      <a
                        ...
                        className="inline-block px-3 py-1 rounded-lg text-xs font-bold bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
                      >
                        PDF
                      </a>
                    )}
